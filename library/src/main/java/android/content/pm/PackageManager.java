...
public abstract class PackageManager {
	...
	/**
	 * {@link PackageInfo} flag: return information about activities in the
	 * package in {@link PackageInfo#activities}.
	 */
	public static final int GET_ACTIVITIES = 0x00000001;

	/**
	 * {@link PackageInfo} flag: return information about intent receivers in
	 * the package in {@link PackageInfo#receivers}.
	 */
	public static final int GET_RECEIVERS = 0x00000002;

	/**
	 * {@link PackageInfo} flag: return information about services in the
	 * package in {@link PackageInfo#services}.
	 */
	public static final int GET_SERVICES = 0x00000004;

	/**
	 * {@link PackageInfo} flag: return information about content providers in
	 * the package in {@link PackageInfo#providers}.
	 */
	public static final int GET_PROVIDERS = 0x00000008;

	/**
	 * {@link PackageInfo} flag: return information about instrumentation in the
	 * package in {@link PackageInfo#instrumentation}.
	 */
	public static final int GET_INSTRUMENTATION = 0x00000010;

	/**
	 * {@link PackageInfo} flag: return information about the intent filters
	...
	/**
	 * {@link ResolveInfo} flag: return the IntentFilter that was matched for a
	 * particular ResolveInfo in {@link ResolveInfo#filter}.
	 */
	...
	/**
	 * {@link PackageInfo} flag: return the {@link PackageInfo#gids group ids}
	 * that are associated with an application. This applies for any API
	 * returning an PackageInfo class, either directly or nested inside of
	 * another.
	 */
	public static final int GET_GIDS = 0x00000100;

	/**
	 * {@link PackageInfo} flag: include disabled components in the returned
	 * info.
	 */
	 ...
	 * that are associated with an application. This applies for any API
	 * returning an ApplicationInfo class, either directly or nested inside of
	 * another.
	 */
	 ...
	 * {@link ProviderInfo#uriPermissionPatterns URI permission patterns} that
	 * are associated with a content provider. This applies for any API
	 * returning an ProviderInfo class, either directly or nested inside of
	 * another.
	 */
	public static final int GET_URI_PERMISSION_PATTERNS = 0x00000800;
	/**
	 * {@link PackageInfo} flag: return information about permissions in the
	 * package in {@link PackageInfo#permissions}.
	 */
	...
	/**
	 * {@link PackageInfo} flag: return information about hardware preferences
	 * in {@link PackageInfo#configPreferences PackageInfo.configPreferences}
	 * and requested features in {@link PackageInfo#reqFeatures
	...
	/**
	 * Permission check result: this is returned by {@link #checkPermission} if
	...
	/**
	 * Signature check result: this is returned by {@link #checkSignatures} if
	 * all signatures on the two packages match.
	 */
	public static final int SIGNATURE_MATCH = 0;

	/**
	 * Signature check result: this is returned by {@link #checkSignatures} if
	 * neither of the two packages is signed.
	 */
	public static final int SIGNATURE_NEITHER_SIGNED = 1;

	/**
	 * Signature check result: this is returned by {@link #checkSignatures} if
	 * the first package is not signed but the second is.
	 */
	public static final int SIGNATURE_FIRST_NOT_SIGNED = -1;

	/**
	 * Signature check result: this is returned by {@link #checkSignatures} if
	 * the second package is not signed but the first is.
	 */
	public static final int SIGNATURE_SECOND_NOT_SIGNED = -2;

	/**
	 * Signature check result: this is returned by {@link #checkSignatures} if
	 * not all signatures on both packages match.
	 */
	public static final int SIGNATURE_NO_MATCH = -3;

	/**
	 * Signature check result: this is returned by {@link #checkSignatures} if
	 * either of the packages are not valid.
	 */
	 ...
	 * Flag for {@link #setApplicationEnabledSetting(String, int, int)} and
	 * {@link #setComponentEnabledSetting(ComponentName, int, int)}: This
	 * component or application has been explictily enabled, regardless of what
	 * it has specified in its manifest.
	 */
	 ...
	 * Flag for {@link #setApplicationEnabledSetting(String, int, int)} and
	 * {@link #setComponentEnabledSetting(ComponentName, int, int)}: This
	 * component or application has been explicitly disabled, regardless of what
	 * it has specified in its manifest.
	 */
	public static final int COMPONENT_ENABLED_STATE_DISABLED = 2;

	/**
	 * Flag for {@link #setApplicationEnabledSetting(String, int, int)} only:
	 * The user has explicitly disabled the application, regardless of what it
	 * has specified in its manifest. Because this is due to the user's request,
	 ...
	 * Be careful when you set this since changing component states can make the
	 * containing application's behavior unpredictable.
	 */
	...
	/**
	 * Flag parameter for {@link #movePackage} to indicate that the package
	...
	/**
	 * Flag parameter for {@link #movePackage} to indicate that the package
	 * should be moved to external media.
	 * 
	 * @hide
	 */
	 ...
	 * more suitable for audio applications sensitive to delays or lag in sound
	 * input or output.
	 */
	 ...
	 * 
	 * @param packageName
	 *            The full name (i.e. com.google.apps.contacts) of the desired
	 *            package.
	 * @param flags
	 ...
	 * @return Returns an array of the same size as the original, containing the
	 *         canonical name for each package.
	 */
	 ...
	 * @return Returns an array of the same size as the original, containing the
	 *         current name for each package.
	 */
	public abstract String[] canonicalToCurrentPackageNames(String[] names);

	/**
	 * Return a "good" intent to launch a front-door activity in a package, for
	 * use for example to implement an "open" button when browsing through
	 * packages. The current implementation will look first for a main activity
	 * in the category {@link Intent#CATEGORY_INFO}, next for a main activity in
	 * the category {@link Intent#CATEGORY_LAUNCHER}, or return null if neither
	 * are found.
	 * 
	 * <p>
	 * Throws {@link NameNotFoundException} if a package with the given name can
	 ...
	 *         launch the main activity in the package, or null if the package
	 ...
	 * 
	 * <p>
	 * Throws {@link NameNotFoundException} if a permission with the given name
	 * can not be found on the system.
	 * 
	 * @param name
	 *            The fully qualified name (i.e. com.google.permission.LOGIN) of
	 *            the permission you are interested in.
	 * @param flags
	 *            Additional option flags. Use {@link #GET_META_DATA} to
	 ...
	 * @param group
	 *            The fully qualified name (i.e. com.google.permission.LOGIN) of
	 *            the permission group you are interested in. Use null to find
	 *            all of the permissions not associated with a group.
	 * @param flags
	 *            Additional option flags. Use {@link #GET_META_DATA} to
	 ...
	 *         information about the package. If flag GET_UNINSTALLED_PACKAGES
	 *         is set and if the package is not found in the list of installed
	 *         applications, the application information is retrieved from the
	 *         list of uninstalled applications(which includes installed
	 *         applications as well as applications with data directory ie
	 *         applications which had been deleted with DONT_DELTE_DATA flag
	 *         set).
	 * 
	 * @see #GET_META_DATA
	 ...
	 * 
	 * <p>
	 * Throws {@link NameNotFoundException} if an activity with the given class
	 * name can not be found on the system.
	 * 
	 * @param component
	 *            The full component name (i.e.
	 *            com.google.apps.contacts/com.google
	 *            .apps.contacts.ContactsList) of an Activity class.
	 * @param flags
	 *            Additional option flags. Use any combination of
	 *            {@link #GET_META_DATA}, {@link #GET_SHARED_LIBRARY_FILES}, to
	 *            modify the data (in ApplicationInfo) returned.
	 * 
	 ...
	 * 
	 * <p>
	 * Throws {@link NameNotFoundException} if a receiver with the given class
	 * name can not be found on the system.
	 * 
	 * @param component
	 *            The full component name (i.e.
	 *            com.google.apps.calendar/com.google
	 *            .apps.calendar.CalendarAlarm) of a Receiver class.
	 * @param flags
	 *            Additional option flags. Use any combination of
	 *            {@link #GET_META_DATA}, {@link #GET_SHARED_LIBRARY_FILES}, to
	 *            modify the data returned.
	 ...
	 * 
	 * <p>
	 * Throws {@link NameNotFoundException} if a service with the given class
	 * name can not be found on the system.
	 * 
	 * @param component
	 *            The full component name (i.e.
	 *            com.google.apps.media/com.google.
	 *            apps.media.BackgroundPlayback) of a Service class.
	 * @param flags
	 *            Additional option flags. Use any combination of
	 *            {@link #GET_META_DATA}, {@link #GET_SHARED_LIBRARY_FILES}, to
	 *            modify the data returned.
	 ...
	 * 
	 * <p>
	 * Throws {@link NameNotFoundException} if a provider with the given class
	 * name can not be found on the system.
	 * 
	 * @param component
	 *            The full component name (i.e.
	 *            com.google.providers.media/com.google
	 *            .providers.media.MediaProvider) of a ContentProvider class.
	 * @param flags
	 *            Additional option flags. Use any combination of
	 *            {@link #GET_META_DATA}, {@link #GET_SHARED_LIBRARY_FILES}, to
	 *            modify the data returned.
	 ...
	 *         including those deleted with DONT_DELETE_DATA (partially
	 ...
	 * member of the permission tree "com.foo".
	 * 
	 * <p>
	 * It is good to make your permission tree name descriptive, because you are
	 * taking possession of that entire set of permission names. Thus, it must
	 * be under a domain you control, with a suffix that will not match any
	 * normal permissions that may be declared in any applications that are part
	 * of that domain.
	 * 
	 * <p>
	 * New permissions must be added before any .apks are installed that use
	 * those permissions. Permissions you add through this method are remembered
	 * across reboots of the device. If the given permission already exists, the
	 * info you supply here will be used to update it.
	 * 
	 * @param info
	...
	/**
	 * Like {@link #addPermission(PermissionInfo)} but asynchronously persists
	 * the package manager state after returning from the call, allowing it to
	 * return quicker and batch a series of adds at the expense of no guarantee
	 * the added permission will be retained if the device is rebooted before it
	 * is written.
	 */
	 ...
	 * appears in both of them. If they do contain the same signature, then they
	 * are allowed special privileges when working with each other: they can
	 * share the same user-id, run instrumentation against each other, etc.
	 * 
	 * @param pkg1
	 ...
	 * 
	 * @return Returns an integer indicating whether all signatures on the two
	 *         packages match. The value is >= 0 ({@link #SIGNATURE_MATCH}) if
	 *         all signatures match or < 0 if there is not a match (
	 *         {@link #SIGNATURE_NO_MATCH} or {@link #SIGNATURE_UNKNOWN_PACKAGE}
	 *         ).
	 * 
	...
	/**
	 * Like {@link #checkSignatures(String, String)}, but takes UIDs of the two
	 * packages to be checked. This can be useful, for example, when doing the
	 * check in an IPC, where the UID is the only identity available. It is
	 * functionally identical to determining the package associated with the
	 * UIDs and checking their signatures.
	 * 
	 * @param uid1
	 ...
	 *            associated packages.
	 * 
	 * @return Returns an array of one or more packages assigned to the user id,
	 *         or null if there are no known packages with the given id.
	 */
	public abstract String[] getPackagesForUid(int uid);

	/**
	 * Retrieve the official name associated with a user id. This name is
	 * guaranteed to never change, though it is possibly for the underlying user
	 * id to be changed. That is, if you are storing information about user ids
	 * in persistent storage, you should use the string returned by this
	 * function instead of the raw user-id.
	 * 
	 * @param uid
	 *            The user id for which you would like to retrieve a name.
	 * @return Returns a unique name for the given user id, or null if the user
	 *         id is not currently assigned.
	 */
	 ...
	 * end up using a common uid. This might be used for new applications that
	 * use an existing shared user name and need to know the uid of the shared
	 * user.
	 * 
	 * @param sharedUserName
	 *            The shared user name whose uid is to be retrieved.
	 * @return Returns the uid associated with the shared user, or
	 *         NameNotFoundException if the shared user name is not being used
	 *         by any installed packages
	 * @hide
	 */
	 ...
	 *         no installed applications, an empty list is returned. If flag
	 *         GET_UNINSTALLED_PACKAGES is set, a list of all applications
	 *         including those deleted with DONT_DELETE_DATA (partially
	 *         installed apps with data directory) will be returned.
	 * 
	 ...
	 * 
	 * @return An array of shared library names that are available on the
	 *         system, or null if none are installed.
	 * 
	 */
	 ...
	 * 
	 * @return An array of FeatureInfo classes describing the features that are
	 *         available on the system, or null if there are none(!!).
	 */
	public abstract FeatureInfo[] getSystemAvailableFeatures();

	/**
	 * Check whether the given feature name is one of the available features as
	 * returned by {@link #getSystemAvailableFeatures()}.
	 * 
	 ...
	 * 
	 * @param intent
	 *            An intent containing all of the desired specification (action,
	 *            data, type, category, and/or component).
	 * @param flags
	 ...
	 *         was determined to be the best action. Returns null if no matching
	 *         activity was found. If multiple matching activities are found and
	 *         there is no default set, returns a ResolveInfo containing
	 ...
	 *         Activity. These are ordered from best to worst match -- that is,
	 *         the first item in the list is what is returned by
	 ...
	 * 
	 * @param processName
	 *            If non-null, limits the returned providers to only those that
	 *            are hosted by the given process. If null, all content
	 ...
	 * 
	 * <p>
	 * Throws {@link NameNotFoundException} if instrumentation with the given
	 * class name can not be found on the system.
	 * 
	 * @param className
	 *            The full name (i.e. com.google.apps.contacts.InstrumentList)
	 *            of an Instrumentation class.
	 * @param flags
	 *            Additional option flags. Currently should always be 0.
	 * 
	 ...
	 * various package manager info structures (such as {@link ComponentInfo} to
	 * implement retrieval of their associated icon.
	 * 
	 * @param packageName
	 *            The name of the package that this icon is coming from. Can not
	 *            be null.
	 * @param resid
	 *            The resource identifier of the desired image. Can not be 0.
	 * @param appInfo
	 *            Overall information about <var>packageName</var>. This may be
	 *            null, in which case the application information will be
	 *            retrieved for you if needed; if you already have this
	 *            information around, it can be much more efficient to supply it
	 *            here.
	 * 
	 ...
	 * icon. If the activity can not be found, NameNotFoundException is thrown.
	 * 
	 * @param activityName
	 *            Name of the activity whose icon is to be retrieved.
	 * 
	 * @return Returns the image of the icon, or the default activity icon if it
	 *         could not be found. Does not return null.
	 * @throws NameNotFoundException
	 *             Thrown if the resources for the given activity could not be
	 *             loaded.
	 * 
	 ...
	 * component. If intent.getClassName() can not be found or the Intent can
	 * not be resolved to a component, NameNotFoundException is thrown.
	 * 
	 * @param intent
	 ...
	 *         could not be found. Does not return null.
	 * @throws NameNotFoundException
	 *             Thrown if the resources for application matching the given
	 *             intent could not be loaded.
	 * 
	 ...
	 * 
	 * @return Returns the image of the icon, or the default application icon if
	 ...
	 * logo. If the activity can not be found, NameNotFoundException is thrown.
	 * 
	 * @param activityName
	 ...
	 *            Information about application being queried.
	 * 
	 * @return Returns the image of the logo, or null if no logo is specified by
	 *         the application.
	 * 
	 ...
	 * 
	 * @return Returns the image of the logo, or null if no application logo has
	 *         been specified.
	 * 
	 * @throws NameNotFoundException
	 *             Thrown if the resources for the given application could not
	 ...
	 * package manager info structures (such as {@link ComponentInfo} to
	 * implement retrieval of their associated labels and other text.
	 * 
	 * @param packageName
	 *            The name of the package that this text is coming from. Can not
	 *            be null.
	 * @param resid
	 *            The resource identifier of the desired text. Can not be 0.
	 * @param appInfo
	 *            Overall information about <var>packageName</var>. This may be
	 *            null, in which case the application information will be
	 *            retrieved for you if needed; if you already have this
	 ...
	 * 
	 * @param packageName
	 *            The name of the package that this xml is coming from. Can not
	 *            be null.
	 * @param resid
	 *            The resource identifier of the desired xml. Can not be 0.
	 * @param appInfo
	 *            Overall information about <var>packageName</var>. This may be
	 *            null, in which case the application information will be
	 *            retrieved for you if needed; if you already have this
	 ...
	 *            here.
	 * 
	 * @return Returns an XmlPullParser allowing you to parse out the XML data.
	 *         Returns null if the xml resource could not be found for any
	 *         reason.
	 */
	 ...
	 * 
	 * @return Returns the label associated with this application, or null if it
	...
	/**
	 * Retrieve the resources associated with an activity. Given the full name
	 * of an activity, retrieves the information about it and calls
	 * getResources() to return its application's resources. If the activity can
	 * not be found, NameNotFoundException is thrown.
	 * 
	 * @param activityName
	 ...
	 * @throws NameNotFoundException
	 *             Thrown if the resources for the given application could not
	 *             be loaded (most likely because it was uninstalled).
	 */
	 ...
	 *            Package name of the application whose resources are to be
	 *            retrieved.
	 * 
	 * @return Returns the application's Resources.
	 * @throws NameNotFoundException
	 ...
	 * 
	 * @return Returns the information about the package. Returns null if the
	 *         package could not be successfully parsed.
	 * 
	 * @see #GET_ACTIVITIES
	 * @see #GET_GIDS
	 * @see #GET_CONFIGURATIONS
	 ...
	 * 
	 * @param packageURI
	 *            The location of the package file to install. This can be a
	 ...
	 * @param flags
	 *            - possible values: {@link #INSTALL_FORWARD_LOCK},
	 *            {@link #INSTALL_REPLACE_EXISTING}, {@link #INSTALL_ALLOW_TEST}
	 *            .
	 * @param installerPackageName
	 *            Optional package name of the application that is performing
	 ...
	 *            The location of the supplementary verification file. This can
	 ...
	 *            either {@link PackageManager#VERIFICATION_ALLOW} or
	 *            {@link PackageManager#VERIFICATION_REJECT}.
	 */
	 ...
	 * limitations on how the installer package can be changed; in particular:
	 * <ul>
	 ...
	 * <li>A SecurityException will be thrown if <var>targetPackage</var>
	 * already has an installer package, and that installer package is not
	 * signed with the same certificate as the calling application.
	 * </ul>
	 * 
	 * @param targetPackage
	 *            The installed package whose installer will be changed.
	 * @param installerPackageName
	 *            The package name of the new installer. May be null to clear
	 *            the association.
	 */
	...
	/**
	 * Attempts to clear the user data directory of an application. Since this
	 * may take a little while, the result will be posted back to the given
	 * observer. A deletion will fail if the named package cannot be found, or
	 * if the named package is a "system package".
	 * 
	 * @param packageName
	 *            The name of the package
	 * @param observer
	 *            An observer callback to get notified when the operation is
	 *            finished
	...
	/**
	 * Attempts to delete the cache files associated with an application. Since
	 * this may take a little while, the result will be posted back to the given
	 * observer. A deletion will fail if the calling context lacks the
	 * {@link android.Manifest.permission#DELETE_CACHE_FILES} permission, if the
	 * named package cannot be found, or if the named package is a
	 * "system package".
	 * 
	 * @param packageName
	 *            The name of the package to delete
	 * @param observer
	 *            An observer callback to get notified when the cache file
	 ...
	 * applications. If the currently available free storage on the device is
	 * greater than or equal to the requested free storage, no cache files are
	 * cleared. If the currently available storage on the device is less than
	 * the requested free storage, some or all of the cache files across all
	 * applications are deleted (based on last accessed time) to increase the
	 * free storage space on the device to the requested value. There is no
	 * guarantee that clearing all the cache files from all applications will
	 * clear up enough storage to achieve the desired value.
	 * 
	 * @param freeStorageSize
	 *            The number of bytes of storage to be freed by the system. Say
	 *            if freeStorageSize is XX, and the current free storage is YY,
	 *            if XX is less than YY, just return. if not free XX-YY number
	 *            of bytes if possible.
	 * @param observer
	 *            call back used to notify when the operation is completed
	 * 
	 * @hide
	 */
	 ...
	 *            of bytes if possible.
	 * @param pi
	 *            IntentSender call back used to notify when the operation is
	 *            completed.May be null to indicate that no call back is
	 *            desired.
	 * 
	 * @hide
	 */
	 ...
	 * context should have the
	 * {@link android.Manifest.permission#GET_PACKAGE_SIZE} permission.
	 * 
	 * @param packageName
	 *            The name of the package whose size information is to be
	 *            retrieved
	 * @param observer
	 *            An observer callback to get notified when the operation is
	 *            complete.
	 *            {@link android.content.pm.IPackageStatsObserver#onGetStatsCompleted(PackageStats, boolean)}
	 *            The observer's callback is invoked with a PackageStats
	 *            object(containing the code, data and cache sizes of the
	 *            package) and a boolean value representing the status of the
	 *            operation. observer may be null to indicate that no callback
	 *            is desired.
	 * 
	 * @hide
	 */
	...
	/**
	 * @deprecated This function no longer does anything; it was an old approach
	 *             to managing preferred activities, which has been superceeded
	 ...
	 *            {@link #GET_SIGNATURES}, to modify the data returned.
	 * 
	 * @return Returns a list of PackageInfo objects describing each preferred
	 ...
	 *             to third party applications. It is the platform's
	 *             responsibility for assigning preferred activities and this
	 *             can not be directly modified.
	 * 
	 *             Add a new preferred activity mapping to the system. This will
	 *             be used to automatically select the given activity component
	 *             when {@link Context#startActivity(Intent)
	 *             Context.startActivity()} finds multiple matching activities
	 *             and also matches the given filter.
	 * 
	 * @param filter
	 *            The set of intents under which this activity will be made
	 *            preferred.
	 * @param match
	 *            The IntentFilter match category that this preference applies
	 *            to.
	 * @param set
	 *            The set of activities that the user was picking from when this
	 *            preference was made.
	 * @param activity
	 ...
	 *             can not be directly modified.
	 * 
	 *             Replaces an existing preferred activity mapping to the
	 *             system, and if that were not present adds a new preferred
	 *             activity. This will be used to automatically select the given
	 *             activity component when {@link Context#startActivity(Intent)
	 *             Context.startActivity()} finds multiple matching activities
	 *             and also matches the given filter.
	 * 
	 * @param filter
	 ...
	 * {@link #addPreferredActivity}, from the system whose activities are
	 * implemented in the given package name. An application can only clear its
	 * own package(s).
	 * 
	 * @param packageName
	 *            The name of the package whose preferred activity mappings are
	 *            to be removed.
	 */
	 ...
	 * {@link #addPreferredActivity}, that are currently registered with the
	 * system.
	 * 
	 * @param outFilters
	 *            A list in which to place the filters of all of the preferred
	 *            activities, or null for none.
	 * @param outActivities
	 *            A list in which to place the component names of all of the
	 *            preferred activities, or null for none.
	 * @param packageName
	 *            An option package in which you would like to limit the list.
	 *            If null, all activities will be returned; if non-null, only
	 *            those activities in the given package are returned.
	 * 
	 * @return Returns the total number of registered preferred activities (the
	 *         number of distinct IntentFilter records, not the number of unique
	 ...
	 * @param newState
	 *            The new enabled state for the component. The legal values for
	 ...
	 * @param flags
	 *            Optional behavior flags: {@link #DONT_KILL_APP} or 0.
	 */
	public abstract void setComponentEnabledSetting(
			ComponentName componentName, int newState, int flags);
	 ...
	 * @return Returns the current enabled state for the component. May be one
	 *         of {@link #COMPONENT_ENABLED_STATE_ENABLED},
	 ...
	 * enabled state which may have been set by the application in its manifest.
	 * It also overrides the enabled state set in the manifest for any of the
	 * application's components. It does not override any enabled state set by
	 * {@link #setComponentEnabledSetting} for any of the application's
	 * components.
	 * 
	 * @param packageName
	 ...
	 *            {@link #COMPONENT_ENABLED_STATE_DISABLED} and
	 *            {@link #COMPONENT_ENABLED_STATE_DEFAULT} The last one removes
	 *            the setting, thereby restoring the applications's state to
	...
	/**
	 * Return the the enabled setting for an application. This returns the last
	 * value set by {@link #setApplicationEnabledSetting(String, int, int)}; in
	 * most cases this value will be {@link #COMPONENT_ENABLED_STATE_DEFAULT}
	 * since the value originally specified in the manifest has not been
	 * modified.
	 * 
	 * @param packageName
	 *            The component to retrieve.
	 ...
	 *            indicate that no callback is desired.
	 * @param flags
	 *            To indicate install location {@link #MOVE_INTERNAL} or
	 *            {@link #MOVE_EXTERNAL_MEDIA}
	 * 
	 * @hide
	 */
	public abstract void movePackage(String packageName,
			IPackageMoveObserver observer, int flags);
	 ...
	 *            the user's name
	 * @param flags
	 *            flags that identify the type of user and other properties.
	 * @see UserInfo
	 * 
	 * @return the UserInfo object for the created user, or null if the user
	...
	/**
	 * @param id
	 *            the ID of the user, where 0 is the primary user.
	 * @hide
	 */
	...
	/**
	 * Returns the app id (or base uid) for a given uid, stripping out the user
	 * id from it.
	 * 
	 * @hide
	 */
